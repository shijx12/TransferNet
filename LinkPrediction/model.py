...
class TransferNet(nn.Module):
    ...
    def forward(self, start, query, answer = None):
        ...
        q_emb = self.dropout(self.relation_embeddings(query))
        ...
        for t in range(self.num_steps):
            ...
            if self.path_type == 'history':
                ...
                for i in range(bsz):
                    ...
                    # Must load simple checkpoint, and detach cq_t.
                    ...
                    rel_feat = self.dropout(self.path_embeddings(rel)) # [rsz, dim_h]
                    ...
                    #         )
                    #     ) # [rsz,]
                    trans_prob = self.transfer_classifier(
                        torch.cat((sub_feat * cq_t[i:i+1], rel_feat), dim=1)
                        ).squeeze(1) # [rsz,]
                    ...
                    new_hist = new_hist / (obj_ep.detach().unsqueeze(1) + 1e-6)
